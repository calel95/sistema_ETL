...
if 'df' in st.session_state:  # Verifica se o DataFrame foi carregado
    transform = Transform(st.session_state['df'])

    if st.button("Remover Duplicatas"):
        #transform = transform.remove_data_duplicates()
        st.session_state['df'] = transform.remove_data_duplicates(df)
    ...
    if st.button("Remover Nulos"):
        if columns_to_check:
            ...
            transform = Transform(st.session_state['df']) 
            st.session_state['df'] = transform.remove_data_nulls(columns_list)
    ...
    if st.button("Filtrar query"):
        if query:
            transform = Transform(st.session_state['df'])
            st.session_state['df'] = transform.filter_select(query,st.session_state['df'],True)
